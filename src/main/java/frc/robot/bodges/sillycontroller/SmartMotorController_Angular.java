...
public class SmartMotorController_Angular {
    ...
    private final PositionVoltage talon_position = new PositionVoltage(0, 0, false, 0, 0, false, false, false);
    private final VelocityVoltage talon_velocity = new VelocityVoltage(0, 0, false, 0, 0, false, false, false);

    private SmartMotorControllerConfiguration_Angular config;
    ...
    public SmartMotorController_Angular(DownstreamControllerType _type, int can_id, SmartMotorControllerConfiguration_Angular _config) {
        this.type = _type;
        this.config = _config;
    ...
    public SmartMotorController_Angular(CANSparkMax downstream, SmartMotorControllerConfiguration_Angular _config) {
    ...
    public SmartMotorController_Angular(TalonFX downstream, SmartMotorControllerConfiguration_Angular _config) {
            ...
            case SPARK_MAX_BRUSHLESS:
                ...
                if (config.mechanismPositionPerEncoderAngularPosition != null)
                    spark.getEncoder().setPositionConversionFactor(
                        config.mechanismPositionPerEncoderAngularPosition.in(Units.Degrees.per(Units.Revolutions))
                    );
                if (config.mechanismVelocityPerEncoderAngularVelocity != null)
                    spark.getEncoder().setVelocityConversionFactor(
                        config.mechanismVelocityPerEncoderAngularVelocity.in(Units.DegreesPerSecond.per(Units.RPM))
                    );
                break;
            case TALON_FX:
                ...
                if (config.mechanismPositionPerEncoderAngularPosition != null) {
                    FeedbackConfigs feedbackConfigs = new FeedbackConfigs()
                        ...
                        .withSensorToMechanismRatio(
                            1.0/config.mechanismPositionPerEncoderAngularPosition.in(Units.Degrees.per(Units.Revolutions))
                        );
    ...
    public void setPIDF_raw(double P, double I, double D, double F) {
        switch (type) {
            ...
            case TALON_FX:
                ...
                talon_config.Slot0.kP = P;
                talon_config.Slot0.kI = I;
                talon_config.Slot0.kD = D;
                talon_config.Slot0.kS = F;
                talon.getConfigurator().apply(talon_config);
                break;
            default:
                break;
        }
     ...
     * requested.
     */
    public void setPIDF_position(PIDF<Angle> pidf) {
        config.pidf_position = pidf;
        positionControlConfigured = true;
        setPIDF_raw(
            pidf.kP == null ? 0 : pidf.kP.in(Units.Volts.per(Units.Degrees))
            , pidf.kI == null ? 0 : pidf.kI.in(Units.Volts.mult(Units.Seconds).per(Units.Degrees))
            , pidf.kD == null ? 0 : pidf.kD.in(Units.Volts.per(Units.DegreesPerSecond))
     ...
     */
    public void setPD_position(PIDF<Angle> pidf) {
        setPIDF_raw(
            pidf.kP.in(Units.Volts.per(Units.Degrees))
            , 0
            , pidf.kD.in(Units.Volts.per(Units.DegreesPerSecond))
            , 0
        );
    }
     ...
     * requested.
     */
    public void setPIDF_velocity(PIDF<Velocity<Angle>> pidf) {
        config.pidf_velocity = pidf;
        velocityControlConfigured = true;
        setPIDF_raw(
            pidf.kP == null ? 0 : pidf.kP.in(Units.Volts.per(Units.DegreesPerSecond))
            , pidf.kI == null ? 0 : pidf.kI.in(Units.Volts.mult(Units.Seconds).per(Units.DegreesPerSecond))
            , pidf.kD == null ? 0 : pidf.kD.in(Units.Volts.per(Units.DegreesPerSecond.per(Units.Second)))
            , pidf.kF == null ? 0 : pidf.kF.in(Units.Volts)
        );
    }
     ...
     */
    public void setPD_velocity(PIDF<Velocity<Angle>> pidf) {
        setPIDF_raw(
            pidf.kP.in(Units.Volts.per(Units.DegreesPerSecond))
            , 0
            , pidf.kD.in(Units.Volts.per(Units.DegreesPerSecond.per(Units.Second)))
            , 0
        );
    }
    ...
    public void setPosition(Measure<Angle> r) {
        ...
        switch (type) {
            ...
            case TALON_FX:
            // TODO verify units?
                talon.setControl(talon_position.withPosition(r.in(Units.Degrees)));
                break;
            default:
                break;
        }
    ...
    public void setVelocity(Measure<Velocity<Angle>> r) {
        ...
        switch (type) {
            ...
            case TALON_FX:
            // TODO verify units?
                talon.setControl(talon_velocity.withVelocity(r.in(Units.DegreesPerSecond)));
                break;
            default:
                break;
        }
    ...
    public Measure<Angle> getPosition() {
        switch (type) {
            ...
            case TALON_FX:
                return Units.Degrees.of(talon.getPosition().getValueAsDouble());
            default:
    ...
    public Measure<Velocity<Angle>> getVelocity() {
        switch (type) {
            ...
            case TALON_FX:
                return Units.DegreesPerSecond.of(talon.getVelocity().getValueAsDouble());
            default:
